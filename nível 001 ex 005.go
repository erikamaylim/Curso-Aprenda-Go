...
func main() {
    fmt.Printf("Valor de x é %v e seu tipo é %T\n", x, x)
    x = 42
    fmt.Printf("x agora possui o valor %v e continua sendo do tipo %T\n", x, x)
    y = int(x)
    fmt.Printf("y recebeu o valor de x com seu tipo convertido. Seu valor e tipo são, respectivamente, %v e %T", y, y)
}